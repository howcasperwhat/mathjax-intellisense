# https://gitlab.com/VladyslavUsenko/basalt-headers

r"""
Uniform cummulative B-spline for SO(3) of order N

For example, in the particular case scalar values and order N=5, for a time
:math:`t \in [t_i, t_{i+1})` the value of :math:`p(t)` depends only on 5 control
points at :math:`[t_i, t_{i+1}, t_{i+2}, t_{i+3}, t_{i+4}]`. To
simplify calculations we transform time to uniform representation :math:`s(t) =
(t - t_0)/\Delta t`, such that control points transform into :math:`s_i \in
[0,..,N]`. We define function :math:`u(t) = s(t)-s_i` to be a time since
the start of the segment. Following the cummulative matrix representation of
De Boor - Cox formula, the value of the function can be evaluated as
follows:

.. math::
  R(u(t)) &= R_i
  \prod_{j=1}^{4}{\exp(k_{j}\log{(R_{i+j-1}^{-1}R_{i+j})})},
  \\ \begin{pmatrix} k_0 \\ k_1 \\ k_2 \\ k_3 \\ k_4 \end{pmatrix}^T &=
  M_{c5} \begin{pmatrix} 1 \\ u \\ u^2 \\ u^3 \\ u^4
  \end{pmatrix},

where :math:`R_{i} \in SO(3)` are knots and :math:`M_{c5}` is a cummulative
blending matrix computed using \ref computeBlendingMatrix 

.. math::
  M_{c5} = \frac{1}{4!}
  \begin{pmatrix} 24 & 0 & 0 & 0 & 0 \\ 23 & 4 & -6 & 4 & -1 \\ 12 & 16 & 0
  & -8 & 3 \\ 1 & 4 & 6 & 4 & -3 \\ 0 & 0 & 0 & 0 & 1 \end{pmatrix}.
"""

"""
Uniform cummulative B-spline for SO(3) of order N

For example, in the particular case scalar values and order N=5, for a time
:math:`t \in [t_i, t_{i+1})` the value of :math:`p(t)` depends only on 5 control
points at :math:`[t_i, t_{i+1}, t_{i+2}, t_{i+3}, t_{i+4}]`. To
simplify calculations we transform time to uniform representation :math:`s(t) =
(t - t_0)/\Delta t`, such that control points transform into :math:`s_i \in
[0,..,N]`. We define function :math:`u(t) = s(t)-s_i` to be a time since
the start of the segment. Following the cummulative matrix representation of
De Boor - Cox formula, the value of the function can be evaluated as
follows:

.. math::
  R(u(t)) &= R_i
  \prod_{j=1}^{4}{\exp(k_{j}\log{(R_{i+j-1}^{-1}R_{i+j})})},
  \\\\ \begin{pmatrix} k_0 \\\\ k_1 \\\\ k_2 \\\\ k_3 \\\\ k_4 \end{pmatrix}^T &=
  M_{c5} \begin{pmatrix} 1 \\\\ u \\\\ u^2 \\\\ u^3 \\\\ u^4
  \end{pmatrix},

where :math:`R_{i} \in SO(3)` are knots and :math:`M_{c5}` is a cummulative
blending matrix computed using \ref computeBlendingMatrix 

.. math::
  M_{c5} = \frac{1}{4!}
  \begin{pmatrix} 24 & 0 & 0 & 0 & 0 \\\\ 23 & 4 & -6 & 4 & -1 \\\\ 12 & 16 & 0
  & -8 & 3 \\\\ 1 & 4 & 6 & 4 & -3 \\\\ 0 & 0 & 0 & 0 & 1 \end{pmatrix}.
"""